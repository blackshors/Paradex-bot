...
class HedgeEngine:
    ...
    def is_not_exist_order(self,paradex_instance,crypto):
        order = paradex_instance.api_client.fetch_orders({"market":crypto})['results']
        next = False
        sleep = 0
        while(len(order)<=0):
            order = paradex_instance.api_client.fetch_orders({"market":crypto})['results']
...
# 随机获取对冲账号
    ...
    def run(self,accounts,exclude):
        ...
        for round_num in range(1, self.ROUNDS + 1):
            ...
            #查询盈亏
            # buy_funding = paradex_instances[a_long].api_client.fetch_fills({"market":crypto})['results'][0] 
            # sell_funding = paradex_instances[a_short].api_client.fetch_fills({"market":crypto})['results'][0] 
            # buy_fee = float(buy_funding['fee'])
            # buy_realized_funding = float(buy_funding['realized_funding'])
            # sell_fee = float(sell_funding['fee'])
            # sell_realized_funding = float(sell_funding['realized_funding'])
            # while((buy_fee+sell_fee)<0):
            #     time.sleep(2)
            #     buy_funding = paradex_instances[a_long].api_client.fetch_fills({"market":crypto})['results'][0] 
            #     sell_funding = paradex_instances[a_short].api_client.fetch_fills({"market":crypto})['results'][0] 
            #     buy_fee = float(buy_funding['fee'])
